...
locals {
  ...
  combined = {
    ...
    azurerm_firewalls                = merge(local.remote.azurerm_firewalls, map(var.landingzone.key, module.caf.azurerm_firewalls))
    public_ip_addresses              = merge(local.remote.public_ip_addresses, map(var.landingzone.key, module.caf.public_ip_addresses))
    virtual_wans                     = merge(local.remote.virtual_wans, map(var.landingzone.key, module.caf.virtual_wans))
